...
void FST::handleMessage(cMessage *msg){
    ...
    if(strcmp(lmsg->getName(),"layer") == 0 || strcmp(lmsg->getName(), "new_layer") == 0){  // Mesaj layer(l) ve ya layer(l+1) mi diye bak
        if(lmsg->getArrivalGateId() != -1){                                                 // Mesajýn Root Düðümden mi geldiðine bak
            ...
            if(par("my_layer").doubleValue() > layer){                                   // bir düðüm layer mesajý aldýðýnda gelen mesajýn maliyeti daha az mý diye kontrol et
                parent = true;
...
layerM *FST::generateMessage(const char *name,short int kind,double delay)
{
